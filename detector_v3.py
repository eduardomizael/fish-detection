...
# Estados por ID
active_ids   = set()
seen_ids     = set()  # IDs únicos confirmados (cumulativo na sessão)
...
while True:
    ...
    # Contador de peixes no centro superior (cumulativo, IDs únicos)
    try:
        fish_count = len(seen_ids)
        H, W = img_show.shape[:2]
        label = f"Peixes: {fish_count}"
        ...
        cv2.putText(img_show, label, (x, y), font, scale, (0, 0, 0), thickness + 2, cv2.LINE_AA)
        cv2.putText(img_show, label, (x, y), font, scale, (255, 255, 255), thickness, cv2.LINE_AA)
    except Exception:
        pass
